...
<body>
    ...
    class Ball extends g2d.shapes.Circle {
        ...
        constructor(parent, name) {
            ...
            this.transform.setPosition(
                g2d.math.irand(
                    BALL_RADIUS + SPAWN_PADDING,
                    game.viewport.width  - BALL_RADIUS - SPAWN_PADDING
                ),
                g2d.math.irand(
                    BALL_RADIUS + SPAWN_PADDING,
                    game.viewport.height - BALL_RADIUS - SPAWN_PADDING
                ),
            );
    ...
    game.update.connect((dt) => {
        for (let i = 0; i < balls.length; ++i) {
            const ball = balls[i]
            if (!i) {
                ball.velocity = ball.direction.scale(BALL_SPEED*dt/1000);
            }
            ...
            for (let j = i + 1; j < balls.length; ++j) {
                const other = balls[j];
                if (!i) {
                    other.velocity = other.direction.scale(BALL_SPEED*dt/1000);
                }
                let normal = position.sub(other.transform.getPosition());
                const distance = normal.abs;

                if (distance > 0 && distance < 2*BALL_RADIUS) {
                    normal = normal.scale(1/distance);
                    const rvelocity = ball.velocity.sub(other.velocity);
                    const normalCoeff = normal.dot(rvelocity);

                    if (normalCoeff > 0) { continue; }

                    const impulse = normal.scale(2*normalCoeff);
                    ...
                    normal = normal.scale(overlap/2);
                    ball.transform.translate(normal);
                    other.transform.translate(normal.scale(-1));
